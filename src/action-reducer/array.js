...
const replaceArrayItem = (state, action) => {
  if (action.index < 0) {
    return state;
  } else {
    const arr = [...getPathValue(state, action.path)];
    arr[action.index] = action.payload;
    return deepAssignValue(state, arr, action.path);
  }
};

export {addArrayItem, delArrayItem, updateArray, replaceArrayItem};
